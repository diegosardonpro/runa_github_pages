# src/db_manager.py (v3.0)
import os
from supabase import create_client, Client

# --- CONSTANTES DE TABLAS ---
URLS_TABLE = 'urls_para_procesar'
ASSETS_TABLE = 'activos' # Tabla central
METADATA_ARTICLES_TABLE = 'metadata_articulos'
METADATA_IMAGES_TABLE = 'metadata_imagenes' # Preparada para el futuro
LOGS_TABLE = 'ejecuciones_log'

# --- INFRAESTRUCTURA COMO CÓDIGO (IaC) v3.0 ---
SCHEMA_SQL = f"""
...
CREATE TABLE IF NOT EXISTS public.{METADATA_ARTICLES_TABLE} (
    ...
    contenido_html text,
    tags text,
    url_imagen_extraida text, -- La URL de la imagen encontrada en el artículo
    ruta_imagen_local text -- La ruta local después de descargarla
);

-- NUEVA TABLA DE METADATOS PARA IMÁGENES (preparada para el futuro)
...
def setup_database_schema(supabase: Client, logger):
    """Asegura que el schema de la BD (v3.0) esté configurado."""
    logger.info("Verificando/creando schema de base de datos v3.0...")
    try:
        ...
        if "already exists" in str(e):
            ...
            logger.error(f"La función 'eval' no existe en la base de datos. Por favor, créala.")
            raise
        else:
...
def create_master_asset(supabase: Client, logger, source_url_id: int, asset_type: str, url_original: str) -> dict | None:
    """Crea una entrada en la tabla central de activos y devuelve el nuevo activo."""
    logger.info(f"Creando registro de activo maestro para URL ID {source_url_id} con tipo '{asset_type}'...")
    try:
        asset_data = {
...
def save_article_metadata(supabase: Client, logger, asset_id: int, metadata: dict):
    """Guarda los metadatos de un artículo en su tabla especializada."""
    ...
    try:
        ...
        raise # Re-lanzamos la excepción para que el orquestador la maneje

def update_asset_status(supabase: Client, logger, asset_id: int, estado_curacion: str):
    """Actualiza el estado de curación en la tabla central de activos."""
...
# --- Funciones existentes que no cambian ---
